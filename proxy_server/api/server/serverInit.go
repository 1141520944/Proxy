...
import (
	...
	"github.com/1141520944/proxy/server/pkg/util"
...
func (sh *ServerHandler) ServerInit(c *gin.Context) {
	...
	go func() {
		...
		case <-serverCreate:
			...
			ok := sh.user.AddServerNum(userid)
			if !ok {
				zap.L().Info("sh.user.AddServerNum() with false")
			}
	...
	case <-serverDie:
		...
		userid, _ := strconv.ParseInt(sr.UserID, 10, 64)
		ok := sh.user.DoneServerNum(userid)
		if !ok {
			zap.L().Info("sh.user.DoneServerNum() with false")
		}
