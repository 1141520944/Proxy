...
func (m *Mysql) Server_SelectAllByUserID(page, size int, userid int64) ([]*models.Table_Server, int64, error) {
	var re []*models.Table_Server
	var count int64
	err := m.DB.Model(&models.Table_Server{}).Count(&count).Limit(size).Offset((page-1)*size).Where("user_id=?", userid).Find(&re).Error
	return re, count, err
}
