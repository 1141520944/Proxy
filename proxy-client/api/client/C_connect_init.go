...
import (
	...
	"github.com/1141520944/proxy/client/util"
...
func (ch *ClientHandler) ClientConnectServer(c *gin.Context) {
	...
	//查找对应结果-server
	src := fmt.Sprintf("http://%s:%s/server/select/show_port/%s", ip, ip_port, show_port)
	...
	client := &models.ClientConnect{
		ConnectAddr:      fmt.Sprintf("%s:%s", "127.0.0.1", sspr.Data.Connect_port),
